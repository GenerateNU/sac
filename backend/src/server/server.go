...
func clubTagRouter(router fiber.Router, clubTagService services.ClubTagServiceInterface) {
	clubTagController := controllers.NewClubTagController(clubTagService)

	clubsTags := router.Group("/clubs/:id/tags")

	clubsTags.Get("/", clubTagController.GetClubTags)
	clubsTags.Post("/", clubTagController.CreateClubTags)
	clubsTags.Delete("/:tagID", clubTagController.DeleteClubTag)
}
...
func tagRoutes(router fiber.Router, tagService services.TagServiceInterface) {
	...
	tags.Get("/:tagID/clubs", tagController.GetTagClubs)
}
